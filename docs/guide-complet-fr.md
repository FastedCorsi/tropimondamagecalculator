...
Les variantes incompatibles sont retirées progressivement. Une information n'est préremplie automatiquement que si elle est commune à toutes les variantes restantes. Les attaques, talents révélés comme `Intimidate`, objets confirmés comme `Leftovers` après leur soin, niveaux et types Tera servent de preuves. Quand une seule variante reste, le set complet peut être appliqué. Si plusieurs variantes restent possibles, un bouton `Set ?` apparaît sans charger arbitrairement le premier set ; un clic permet ensuite de parcourir manuellement uniquement les variantes encore compatibles. Le survol affiche le contenu de la variante sélectionnée. Les sets strictement identiques sont fusionnés pendant le chargement.

L'équipe Random Battle du joueur est récupérée depuis l'acteur de combat Cobblemon afin de conserver ses formes, niveaux, IV, nature, talent, objet et attaques. Les six statistiques reçoivent systématiquement `85 EV` selon la règle du format, même lorsque Cobblemon expose une table partielle contenant des zéros. Tous les adversaires reçoivent la même répartition d'EV et une nature `Serious`, y compris lorsqu'ils viennent d'un Team Preview déjà résolu. Le calculateur affine ensuite objet, talent, Tera et attaques avec les variantes Tropimon et les révélations du combat.
